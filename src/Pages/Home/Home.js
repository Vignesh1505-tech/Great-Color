        ...
        <div className='grid grid-cols-2 gap-4 my-3 md:grid-cols-4'>
          {
            item.items.map((data) => (
              <div key={data.id}>
                ...
                <Card img={data.img} path={item.title==="Floor sticker"?`/${item.text}/${item.data[0].header!== undefined? item.data[0].header:item.data[0].title}`:`sub/${data.name}`} data={data.data.subproduct} view={true} imgClass={`lg:w-72 h-40 lg:h-64`} text={data.name} />
              </div>
            ))
          }
        </div>
